    ...
    var SlideItClass = function($viewport, opts) {
        ...
        viewport = {
            world: {
                ...
                init: function() {
                    this.$slides = $(opts.slideSelector, $viewport);
                    $viewport.wrapInner('<div/>');
                    this.$elem = $('div:eq(0)', $viewport).css({
                        position: 'absolute',
                        top: 0,
                        right: 0,
                        bottom: 0,
        ...
        events = {
            ...
            onMouseupClick: function() {
                ...
                if (!panUtil.isPanning && opts.events.onClickSlide) {
                    var $slide = $(this),
                        slideIdx = viewport.world.$slides.index($slide.closest(viewport.world.$slides));
                    if (slideIdx > -1) {
                        $viewport.triggerHandler('clickSlide.rsSlideIt', [$slide, slideIdx]);
                    }
                }
            },
            onDblClick: function(event) {
                if (opts.events.onDblClickSlide) {
                    var $slide = $(this),
                        slideIdx = viewport.world.$slides.index($slide.closest(viewport.world.$slides));
                    if (slideIdx > -1) {
                        $viewport.triggerHandler('dblClickSlide.rsSlideIt', [$slide, slideIdx]);
                    }
        ...
        var load = {
                ...
                getNextSibling: function($slide) {
                    ...
                    while ($candidateNext.length === 0 || $candidateNext.closest(viewport.world.$elem).length === 1) {
                        while ($candidateNext.length === 0) {
                            ...
                            if ($candidateNext.is(viewport.world.$slides) || $candidateNext.parent().closest(viewport.world.$elem).length === 0) {
                                return null;
                            }
                        ...
                        if ($next.is(viewport.world.$slides)) {
                            return $next;
                        }
                        $children = $next.find(viewport.world.$slides);
                ...
                getChildren: function($slide) {
                    ...
                    return $slide.
                            find(viewport.world.$slides).
                            filter(function(i, e) {
                                return $(e).parent().closest(viewport.world.$slides).is($slide);
                            });
                },
        ...
        option = function() {
            if (typeof arguments[0] === 'string') {
                switch (arguments.length) {
                    case 1:
                        return this.eq(0).triggerHandler('getter.rsSlideIt', arguments);
                    case 2:
                        for (var last = this.length - 1; last > -1; --last) {
                            this.eq(last).triggerHandler('setter.rsSlideIt', arguments);
                        }
                        return this;
                }
            }
